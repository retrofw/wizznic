...
int initMenu()
{
  sprintf(home_path, "%s/.wizznic", getenv("HOME"));
  ...
  switch(menuState)
  {
    case menuStateIntro:
     ...
     // SDL_BlitSurface(menuBg[MENUGFXINTRO] , NULL, screen, NULL );
      ...
      //Blink "Press A"
      if(dir) txtWriteCenter(screen, FONTSMALL, "Press A", 160, 190);
    ...
    case menuStateNewGame: //Start new game (select starting level)
      ...
      if(selectedMenuItem == selMax)
      {
        ...
        txtWriteCenter(screen, FONTSMALL, "Press A to play", 160,240-12 );

        if(getButton(C_BTNA) && selectedMenuItem!=getNumLevels())
        {
          resetBtn(C_BTNA);
    ...
    case menuStateNextLevel:
      ...
      if(dir) txtWriteCenter(screen, FONTSMALL, "Press A", 160, 180);
    ...
    case menuStatePaused:
      ...
      txtWave(screen, FONTSMALL, buf, 160, 14, &rot2);

      txtWave(screen, FONTMEDIUM, "Pause!", 160, 45, &rot);

      int y = 100;
      if(dir || selectedMenuItem!= MENU_NEW_GAME) txtWriteCenter(screen, FONTSMALL, "New Game", 160, y); y += 20;
      if(dir || selectedMenuItem!= MENU_RESTART) txtWriteCenter(screen, FONTSMALL, "Restart", 160, y); y += 20;
      // if(dir || selectedMenuItem!= MENU_RESUME) txtWriteCenter(screen, FONTSMALL, "Resume", 160, y); y += 20;
      if(dir || selectedMenuItem!= MENU_USER_LEVELS) txtWriteCenter(screen, FONTSMALL, "User Levels", 160, y); y += 20;
      if(dir || selectedMenuItem!= MENU_HELP) txtWriteCenter(screen, FONTSMALL, "Help", 160, y); y += 20;
      if(dir || selectedMenuItem!= MENU_ABOUT) txtWriteCenter(screen, FONTSMALL, "About", 160, y); y += 20;
      if(dir || selectedMenuItem!= MENU_EXIT) txtWriteCenter(screen, FONTSMALL, "Exit", 160, y); y += 20;
      ...
      if( getButton( C_BTNA ) )
      {
        ...
        {
          case MENU_NEW_GAME: //Newgame: Return to game with paused set, it will cleanup, set gameover.
            //Cleanup will save scores
            player()->gameOver=1;
            return(STATEPLAY);
          break;
          case MENU_RESTART: //restart current level
            player()->paused=1; //This makes the gamecode reset
            player()->boardScore=0; //He won't earn any points
            setMenu(menuStateRestart);
            return(STATEPLAY);
          break;
          // case MENU_RESUME: //Resume, unset paused, return to game
          //   player()->paused=0;
          //   return(STATEPLAY);
          // break;
          case MENU_HELP: //Switch to help screen
            player()->paused=0;
            menuState=menuStateHowto;
          break;
          case MENU_USER_LEVELS: //UserLevels
            player()->gameOver=1;
            menuState=menuStateUserLevels;
            return(STATEPLAY);
          break;
          case MENU_ABOUT: //Switch to about screen
            player()->paused=0;
            menuState=menuStateAbout;
          break;
          case MENU_EXIT: //Save scores, Exit program without cleaning up (OS job :P)
      ...
      case menuStateHowto:
        ...
        if(dir) txtWriteCenter(screen, FONTSMALL, "Press A play.", 160, 240-12);
      ...
      case menuStateUserLevels:
        ...
        if(dir || selectedMenuItem!= 0) txtWriteCenter(screen, FONTSMALL, "Create",160,50);
        if(dir || selectedMenuItem!= 1) txtWriteCenter(screen, FONTSMALL, "Exit",160,60);

        //List levels
        int ul=0;
        while(ul < getNumUserLevels())
        {
          sprintf(buf, "UserLevel %i (A=edit) (Y=play)", ul);
        ...
        if(getButton(C_BTNA))
        {
          ...
          if(selectedMenuItem==0)
          {
            ...
            sprintf(buf, "%s/userlevel%i.wzp", home_path, getNumUserLevels());
