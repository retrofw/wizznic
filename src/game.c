...
void drawUi(SDL_Surface* screen)
{
   //Draw text
    ...
    sprintf(tempStr, "Lives: %i", player()->lives);
    txtWriteCenter(screen, FONTSMALL, tempStr, 46, 14);

    sprintf(tempStr, "Retries: %i", player()->retries);
    txtWriteCenter(screen, FONTSMALL, tempStr, 46, 34);
...
int runGame(SDL_Surface* screen)
{
  if(gameState==GAMESTATEPLAYING)
  {
    ...
    if( getButton( C_RIGHT ) )
    {
      ...
      //Drag
      if( getButton( C_BTNX ) || getButton( C_BTNB ) || getButton( C_BTNA ) || getButton( C_BTNY ) )
      {
        //Sound grab brick
