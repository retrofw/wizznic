...
int runEditor(SDL_Surface* screen)
{
  ...
  txtWrite(screen, FONTSMALL,"A: Put", 0,140);
  txtWrite(screen, FONTSMALL,"B: Del", 0,150);
  txtWrite(screen, FONTSMALL,"Start:Save", 0,160);
  txtWrite(screen, FONTSMALL,"Select:Exit", 0,170);

  txtWriteCenter(screen, FONTSMALL, "Brick",45,20);
  drawBrick(screen, selBrick,35,35);
  if(selBrick>1)
    txtWrite(screen, FONTSMALL,"<Y", 35-18-3,38);

  if(selBrick< 8)
    txtWrite(screen, FONTSMALL,"X>", 55+3,38);
